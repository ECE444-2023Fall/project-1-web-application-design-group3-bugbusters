...
from flask_server.global_config import AlgoliaSearchClient
...
class TestEventService(): 
    ...
    #Lab 5 - Chris Mountain Unit Test
    def test_algolia_search(self):
        ...
        #Test
        AlgoliaSearchClient.add_to_index(originial_event_json['_event_id'], originial_event_json)
        results = AlgoliaSearchClient.search_index("SKULE Band Meeting")
        AlgoliaSearchClient.delete_from_index(originial_event_json['_event_id'])

        #Validate
        foundEvent = False
        for hit in results['hits']:
            if hit['event_title'] == 'SKULE Band Meeting':
                foundEvent = True

        assert foundEvent == True
